# 03f — Workflow Engine: POST .../steps/[n]/generate

## What this builds

The generate route: `POST /api/v1/projects/[id]/steps/[n]/generate`.
Handles first generate, retry, and follow-up messages for LLM steps.

## Prerequisites

[03d-llm-builder.md](03d-llm-builder.md) — `buildSystemMessage`, `buildMessages`, `callLLM`.
[03c-interpolate.md](03c-interpolate.md) — `interpolate()`.
[03b-workflow-templates.md](03b-workflow-templates.md) — `getStepDefinition()`.
[03e-api-create-project.md](03e-api-create-project.md) — project must exist to generate.

## Files to create

```
src/app/api/v1/projects/[id]/steps/[n]/generate/route.ts
```
...
```ts
{
  followUpMessage?: string   // if set: add this message and continue conversation
  retry?: boolean            // if true: clear conversation, start fresh
}
```

Both are optional. An empty body `{}` means "first generate" or "regenerate from current state".
...
import { buildSystemMessage, buildMessages, callLLM } from '@/lib/llm'
...
export async function POST(req: Request, { params }: Ctx) {
  ...
  const { followUpMessage, retry } = body as { followUpMessage?: string; retry?: boolean }
  ...
  if (stepDef.type === 'external_instruction') {
    return NextResponse.json({ error: 'External steps do not call the LLM' }, { status: 400 })
  }
...
| Mode | Body | Behavior |
|------|------|----------|
| First generate | `{}` | Conversation empty → calls LLM with just system + prompt |
| Follow-up | `{ followUpMessage: "Make it shorter" }` | Appends user msg to conversation, calls LLM |
| Retry | `{ retry: true }` | Clears conversation + llmResponse, calls LLM fresh |
...
| Code | Meaning |
|------|---------|
| 400 | Invalid step, external step, or step ordering violation |
| 404 | Project not found |
| 409 | Already generating (concurrent call) |
| 502 | LLM API error (OpenAI returned non-200) |

---

**Output:** Working generate route that handles first generate, follow-up, and retry.

**Next step:** [03g-api-approve.md](03g-api-approve.md) — POST .../steps/[n]/approve
