# R12a — Merge 3 Regenerate API Routes → 1
**Parent:** R12-refactor-overview.md  
**Impact:** -280 lines, -2 files  
**Risk:** Low — prompts are different, but the surrounding scaffolding is identical  

---

## Problem

Three route files are 90% copy-paste of each other:

```
src/app/api/v1/projects/[id]/steps/[n]/regenerate-character-prompt/route.ts  (119 lines)
src/app/api/v1/projects/[id]/steps/[n]/regenerate-kling-prompt/route.ts      (129 lines)
src/app/api/v1/projects/[id]/steps/[n]/regenerate-scene-prompt/route.ts      (147 lines)
```

All three do:
1. Validate `n` matches their expected step number (5, 7, 6)
2. Validate `id` is a valid ObjectId
3. Parse body
4. Load project + populate brandProfileId
5. Check prerequisite step is done
6. Build a prompt
7. Call LLM
8. Check Jaccard similarity — if too similar, retry once
9. Return `{ ok: true, prompt: newPrompt }`

The ONLY differences are the step number, prerequisite check, and the prompt text.

Also: `normalizeForSimilarity` and `jaccardSimilarity` are copy-pasted in all three files. These were already extracted to `src/lib/jaccard-similarity.ts` — the route files just don't import it yet.

---

## Solution

Merge into a single route at:
```
src/app/api/v1/projects/[id]/steps/[n]/regenerate-prompt/route.ts
```

The route reads `n` from the URL and dispatches to the right prompt builder.

**Step 1: Create the unified route**

File: `src/app/api/v1/projects/[id]/steps/[n]/regenerate-prompt/route.ts`
...
async function generateWithRetry(
  ...
  oldPrompt: string,
  retryMessages: { role: 'system' | 'user'; content: string }[],
  model: string
): Promise<string> {
...
function buildSceneMessages(
  ...
  return { messages: [{ role: 'system', content: systemMessage }, { role: 'user', content: userPrompt }], retryMessages, referenceUrls }
}
...
function buildKlingMessages(
  ...
  return { messages: [{ role: 'system', content: systemMessage }, { role: 'user', content: userPrompt }], retryMessages }
}
...
**Step 2: Update client-side callers**

Search for all three old endpoint paths in the client code:

```
/regenerate-character-prompt
/regenerate-kling-prompt
/regenerate-scene-prompt
```

Replace all three with `/regenerate-prompt`. The request body shape is unchanged.

Files to check:
- `src/components/CharacterStepPanel.tsx` — calls `/regenerate-character-prompt`
- `src/components/SceneStepPanel.tsx` — calls `/regenerate-scene-prompt`
- `src/components/KlingStepPanel.tsx` — calls `/regenerate-kling-prompt`

**Step 3: Delete old route files**

```
src/app/api/v1/projects/[id]/steps/[n]/regenerate-character-prompt/  (delete dir)
src/app/api/v1/projects/[id]/steps/[n]/regenerate-kling-prompt/      (delete dir)
src/app/api/v1/projects/[id]/steps/[n]/regenerate-scene-prompt/      (delete dir)
```
...
- [ ] POST to `/api/v1/projects/:id/steps/5/regenerate-prompt` with `characterName` + `currentPrompt` returns new prompt
- [ ] POST to `/api/v1/projects/:id/steps/6/regenerate-prompt` with `currentPrompt` returns new prompt with reference URLs preserved
- [ ] POST to `/api/v1/projects/:id/steps/7/regenerate-prompt` with `currentPrompt` returns new prompt
- [ ] POST with invalid step number (e.g. 4) returns 400
- [ ] CharacterStepPanel regenerate button still works
- [ ] SceneStepPanel regenerate button still works
- [ ] KlingStepPanel regenerate button still works
- [ ] Jaccard similarity retry still works (if LLM returns same text, a second attempt is made)
