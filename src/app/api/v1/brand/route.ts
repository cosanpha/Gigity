...
export const POST = apiHandler(
  async (req: Request) => {
    ...
    if (body.name?.length > 200) {
      return NextResponse.json(
        { error: 'name is too long (max 200 characters)', field: 'name' },
        { status: 400 }
      )
    }
    ...
    if (body.targetAudience?.length > 2000) {
      return NextResponse.json(
        {
          error: 'targetAudience is too long (max 2000 characters)',
    ...
    if (body.tone?.length > 500) {
      return NextResponse.json(
        { error: 'tone is too long (max 500 characters)', field: 'tone' },
        { status: 400 }
      )
    }
    // Validate URL fields
